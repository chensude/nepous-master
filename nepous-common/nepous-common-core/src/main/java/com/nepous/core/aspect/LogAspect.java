package com.nepous.core.aspect;

import com.nepous.core.annotation.LogAnnotation;
import com.nepous.core.annotation.OperationLogDto;
import com.nepous.core.utils.RequestUtil;
import com.nepous.dto.LoginAuthDto;
import com.nepous.util.JacksonUtil;
import com.nepous.util.PubUtils;
...
//
    ...
    @Pointcut("@annotation(com.nepous.core.annotation.LogAnnotation)")
    ...
    public void doAfter(final JoinPoint joinPoint, final Object returnValue) {
        if (returnValue instanceof Result) {
            ...
            if (!PubUtils.isNull(result) && result.getCode() == Result.SUCCESS_CODE) {
...
//            operationLogDto.setLastOperatorId(loginUser.getUserId());
            ...
            //持久化到数据库中
            //taskExecutor.execute(() -> this.restTemplate.postForObject("http://nepous-provider-uac/auth/saveLog", operationLogDto, Integer.class));
        } catch (Exception ex) {
